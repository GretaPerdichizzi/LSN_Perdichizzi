...
void Population :: Sorting(){
	bool swapped;
	for(int i=0; i<pop[0].chromo.size();i++){
		swapped = false;
		for(int j=0; j<n_chromo-i-1; j++){
...
double Population :: EvalFitness(){
	double sum=0;
	for(int i=0; int (i<n_chromo); i++ ){
		sum+=pop[i].L1();
	}
	return sum;
}

void Population :: PrintFitness(int gen){
	double sum=0;
	sum=EvalFitness();
	sum/=(n_chromo/2);
	ofstream WriteL;
	WriteL.open("fitness_mean.out", ofstream::app);
