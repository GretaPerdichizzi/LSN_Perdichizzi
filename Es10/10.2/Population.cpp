...
void Population :: GetPop(){
	...
	for (int i=0; i<n_chromo; i++){
		...
		random_shuffle(init.chromo.begin()+1, init.chromo.end()-1);
		pop.push_back(init);
...
void Population :: PrintFitness(int gen, int size, int rank){
		...
		sum/=(n_chromo/2);
		ofstream WriteL;
		WriteL.open("node0_fitness_mean.out", ofstream::app);
		...
		sum/=(n_chromo/2);
		ofstream WriteL;
		WriteL.open("node1_fitness_mean.out", ofstream::app);
		WriteL << gen << setw(12) << sum << setw(12) << pop[0].L1() << endl;
		WriteL.close();
		}
		if(rank==2){
		...
		sum/=(n_chromo/2);
		ofstream WriteL;
		WriteL.open("node2_fitness_mean.out", ofstream::app);
		WriteL << gen << setw(12) << sum << setw(12) << pop[0].L1() << endl;
		WriteL.close();
		}
		if(rank==3){
		...
		sum/=(n_chromo/2);
		ofstream WriteL;
		WriteL.open("node3_fitness_mean.out", ofstream::app);
		WriteL << gen << setw(12) << sum << setw(12) << pop[0].L1() << endl;
		WriteL.close();
		}
}
...
void Population :: GetBest (){
	for (int i=0; i<32; i++){
		best_path_x.push_back(pop[0].chromo[i].x);
	}
	for (int i=0; i<32; i++){
		best_path_y.push_back(pop[0].chromo[i].y);
	}
}

//****************************************************************//

void Population :: CopyBest (){
	for (int i=0; i<32; i++){
		pop[0].chromo[i].x=best_path_x[i];
	}
	for (int i=0; i<32; i++){
		pop[0].chromo[i].y=best_path_y[i];
	}
}
