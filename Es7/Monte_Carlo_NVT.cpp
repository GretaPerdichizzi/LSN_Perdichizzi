...
{ 
  ...
  for(int iblk=1; iblk <= nblk; ++iblk) //Simulation
  {
    ...
    for(int istep=1; istep <= nstep; ++istep)
    {
      Move();
      Measure();
			Instant(0);  //If 1 print results for current step
...
void Instant(bool active)  //Function to print instant properties values
{
	if(active==1)
	{
		ofstream Epot_inst, Pres_inst;
		Epot_inst.open("output.epot_stepbystep_solid.0",ios::app);
		Pres_inst.open("output.pres_stepbystep_solid.0",ios::app);
		Epot_inst << walker[iv]/(double)npart + vtail << endl;
		Pres_inst << rho * temp + (walker[iw] + ptail * (double)npart) / vol <<endl;
		Epot_inst.close();
		Pres_inst.close();
	}
}
