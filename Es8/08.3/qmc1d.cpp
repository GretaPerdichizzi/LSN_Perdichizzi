...
double variationalWaveFunction(double v)
{
	...
	//return exp(-0.5*v*v);
	double psi;
    	psi=exp(-pow((v-mu),2)/(2.*pow(sigma,2)))+exp(-pow((v+mu),2)/(2.*pow(sigma,2)));
    	return psi;
}
...
double variationalWaveFunction_second(double v)
{
	...
	double exp1, exp2;
	double der2psi;
	exp1 = exp(-pow((v-mu),2)/2./pow(sigma,2));
	exp2 = exp(-pow((v+mu),2)/2./pow(sigma,2));
	der2psi = (-exp1-exp2+(exp1*(pow((v-mu),2))/pow(sigma,2))+(exp2*(pow((v+mu),2))/pow(sigma,2)))/pow(sigma,2);
	return der2psi;
}
